...
///ake a seperate array of resultant which only has name and cusine
// const{recipes}=data;
// let arr=[];
...
// const {recipes}=data;
// const newArr=recipes.map((ele)=>{
//   const {name,cuisine}=ele;
//   return {name,cuisine};
// })
...
const {recipes}=data
const newArr=recipes.filter((ele)=>{
  const { rating }=ele;
  if(rating>4.5) return true;
  else return false;
});

const finalArr=newArr.map((ele)=>{
  const {name,cuisine}=ele;
  return{
    name,
    cuisine
